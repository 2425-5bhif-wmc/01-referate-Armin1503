...
Docker Swarm ist ein Container-Orchestrierungsdienst von Docker, der es ermöglicht, mehrere Docker-Hosts (Rechner (physisch oder virtuell), auf dem DOcker läuft) zu einem Cluster (Swarm) zusammenzuschließen.
...
Bei Webanwendungen werden Übersetzungsdateien häufig im JSON-Format gespeichert, aber auch YAML-Dateien können verwendet werden. Diese Dateien sind typischerweise nach der Sprache strukturiert (z. B. en.json für Englisch, de.json für Deutsch).
...
Zuerst müssen die **@ngx-translate/core**- und **@ngx-translate/http-loader**-Bibliothek installiert werden, welche für folgendes benötigt werden:
